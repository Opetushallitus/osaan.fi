...
cd $repo_path
dev-scripts/copy-db.sh osaan osaan_integraatio_backup
if [ "$use_existing_integraatio_database" = "no" ]; then
    dev-scripts/create-db-schema.sh
else
    dev-scripts/copy-db.sh osaan_integraatio osaan
fi
...
cd $repo_path
dev-scripts/copy-db.sh osaan osaan_integraatio
dev-scripts/copy-db.sh osaan_integraatio_backup osaan 
